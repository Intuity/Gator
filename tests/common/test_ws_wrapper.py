...
from gator.common.ws_wrapper import (
    WebsocketWrapper,
...
class TestWebsocketWrapper:
    ...
    async def test_posted(self, wrapper):
        ...
        wrapper.ws.send.assert_called_with(
            json.dumps(
                {
                    "action": "some_route",
                    "posted": True,
                    "payload": {"word": "hello", "name": "fred"},
                }
            )
        )
    ...
    async def test_non_posted(self, wrapper):
        ...
        wrapper._WebsocketWrapper__pending[0].response = {
            "result": "success",
            "action": "some_route",
            "rsp_id": 0,
    ...
    async def test_monitor_fallback(self, wrapper, mocker):
        ...
        message = {"action": "other_route", "payload": {"word": "hello"}}
        await ws_q.put(json.dumps(message))
        # Wait for routing to happen
        await ev_route.wait()
        ev_route.clear()
